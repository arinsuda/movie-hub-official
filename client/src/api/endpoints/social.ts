...
export const reviewApi = {
  ...
  getMediaReviews: (
    ...
    api.get<PaginatedResponse<ReviewResponse>>(`/${mediaType}/${mediaId}/reviews`, {
      params: { page: 1, limit: 20, ...params },
    }),

  updateReview: (userId: number, reviewId: number, data: UpdateReviewRequest) =>
    api.patch<{ review: ReviewResponse }>(`/users/${userId}/reviews/${reviewId}`, data),

  deleteReview: (userId: number, reviewId: number) =>
    api.delete(`/users/${userId}/reviews/${reviewId}`),

  likeReview: (reviewId: number) =>
    api.post(`/reviews/${reviewId}/likes`),

  unlikeReview: (reviewId: number) =>
    api.delete(`/reviews/${reviewId}/likes`),

  getComments: (reviewId: number, page = 1, limit = 20) =>
    api.get<PaginatedResponse<CommentResponse>>(`/reviews/${reviewId}/comments`, {
      params: { page, limit },
    }),

  createComment: (reviewId: number, data: CreateCommentRequest) =>
    api.post<{ comment: CommentResponse }>(`/reviews/${reviewId}/comments`, data),

  updateComment: (reviewId: number, commentId: number, body: string) =>
    api.patch<{ comment: CommentResponse }>(`/reviews/${reviewId}/comments/${commentId}`, { body }),
...
export const libraryApi = {
  ...
  getLibrary: (userId: number, params?: { list_type?: ListType; media_type?: MediaType }) =>
    api.get<{ items: LibraryItemResponse[] }>(`/users/${userId}/library`, { params }),
  ...
  ) => api.patch<{ item: LibraryItemResponse }>(`/users/${userId}/library/${itemId}`, data),
...
export const followApi = {
  ...
  getFollowers: (userId: number, page = 1, limit = 20) =>
    api.get<PaginatedResponse<FollowUserResponse>>(`/users/${userId}/followers`, {
      params: { page, limit },
    }),

  getFollowing: (userId: number, page = 1, limit = 20) =>
    api.get<PaginatedResponse<FollowUserResponse>>(`/users/${userId}/following`, {
      params: { page, limit },
    }),

  getFeed: (page = 1, limit = 20) =>
    api.get<PaginatedResponse<FeedItemResponse>>('/feed', { params: { page, limit } }),
}
